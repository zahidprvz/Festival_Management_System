...
<div class="row">
    ...
    <div class="container">
        <form action="" method="POST">
            <h1>Please enter the fields to register in ENSEMBLE-UOG</h1>
    ...
    if($tright==1)
    {
        if($mright==$i)
        {
            foreach ($user as $u)
            {
                ...
                // $pw=mysqli_real_escape_string($mysqli,$u['password']);
                $t_id=mysqli_real_escape_string($mysqli,$_POST["tname"]);
                $query2="insert into t_participation (t_id,p_id,e_id) values('$t_id','$p_id',4);";
            ...
            if($j==$i)
            {
                echo "<script>alert('Succesfully registered your team in Ensemble')</script>";
            }
